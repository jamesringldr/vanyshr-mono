...
import { parseZabaSummaries } from "./html-scrapers.ts";
...
// ---------------------------------------------------------------------------
// Zaba — the search page IS the profile page. Target one div.person.
// ---------------------------------------------------------------------------

export type ZabaDetailHint = {
  ordinal?: number;
  full_name?: string;
  age?: number;
  address?: string;
...
/** Pick the right zaba card: name+age+address first, ordinal as fallback. */
export function selectZabaSummary(
  summaries: SummaryResult[],
  hint?: ZabaDetailHint,
): SummaryResult | undefined {
  if (!summaries.length) return undefined;
  if (!hint) return summaries[0];

  const named = hint.full_name
    ? summaries.filter((s) => namesEqual(s.full_name, hint.full_name))
    : summaries;

  if (hint.age != null) {
    const aged = named.filter((s) => s.age === hint.age);
    if (aged.length === 1) return aged[0];
    if (aged.length > 1 && hint.address) {
      const needle = hint.address.toLowerCase();
      const byAddr = aged.find((s) => {
        const addr = (s.address || "").toLowerCase();
        return addr && needle && (addr.includes(needle.slice(0, 12)) || needle.includes(addr.slice(0, 12)));
      });
  ...
  if (hint.ordinal != null && summaries[hint.ordinal]) return summaries[hint.ordinal];
  return named[0] ?? summaries[0];
}

export function parseZabaDetail(html: string, hint?: ZabaDetailHint): BrokerDetailProfile {
  const summaries = parseZabaSummaries(html);
  if (summaries.length > 0) {
    const picked = selectZabaSummary(summaries, hint);
    return picked ? summaryFallback(picked) : {};
  }
  ...
  const settled = await Promise.allSettled(
    members.map(async (member) => {
      ...
      try {
        ...
        const detail = broker === BrokerName.ZABA
          ? parseZabaDetail(page.html, {
              ordinal: member.summary.ordinal,
              full_name: member.summary.full_name,
              age: member.summary.age,
              address: member.summary.address,
            })
          : DETAIL_PARSERS[broker]?.(page.html);
