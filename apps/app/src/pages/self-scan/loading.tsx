...
import type { ScanMember } from "../pilot-scan/scan-result";
 ...
 */
export function SelfScanLoadingPage() {
  ...
  // Tip: `/self-scan/loading?noresults` opens the recovery modal without a scan.
  ...
  async function handleScanAgain(type: "first" | "last", value: string) {
    ...
    if (!trimmed || !fields) {
      navigate("/self-scan", { replace: true });
      return;
    }
  ...
  return (
    ...
    >
      ...
      {/* Status container -- fixed (not expandable/collapsible), last child
          of this h-screen column so the zone above gets the rest of the
          height from flexbox, same as the drawer it replaces. */}
      <StatusContainer
        isOpen={drawerOpen}
