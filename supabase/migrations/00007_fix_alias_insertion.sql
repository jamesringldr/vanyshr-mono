-- ============================================================================
-- Migration 00007: Fix alias insertion in create_pending_profile()
-- ============================================================================
-- Bug: Migration 00006 accidentally used jsonb_array_elements + ->>'alias' for
-- the aliases array, but QuickScanProfileData.aliases is a plain string[].
-- Using ->>'alias' on a JSON string returns NULL, so no aliases were ever
-- inserted from quick-scan data.
-- Fix: Use jsonb_array_elements_text() which extracts plain text directly
-- from a JSON string array — matching the data format produced by
-- convertToQuickScanProfileData() in BaseScraper.ts.
-- ============================================================================
...
BEGIN
    ...
    -- QuickScanProfileData.aliases is a plain string[] (e.g. ["John S", "JS"]).
    -- Use jsonb_array_elements_text() to extract each string directly.
    -- ----------------------------------------------------------------
    IF v_scan.profile_data IS NOT NULL
