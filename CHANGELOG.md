...
- AnyWho and FPS detail-page addresses could duplicate the same way as the client-side bug above, at the source: both brokers' current-address elements sometimes contain the whole address with no comma at all, and the code separately re-parsed city/state/zip from a wider container that necessarily re-included that same text, then joined both — doubling it. Fixed by parsing the address through the shared street-suffix-aware parser first. Along the way, found FPS's previous-addresses were broken on 12 of 13 entries on one real profile (not an edge case) — the street name and city were dumped together into "city" whenever no comma marked the boundary — and a related bug in the shared parser itself that lost the city entirely when a unit number and the city shared one comma-delimited chunk (FPS's format). All verified against real saved broker HTML, not synthetic guesses.
- National Public Data's full-profile scraper was badly broken: name came out as "James Oehring, 62." (age and a stray period glued onto it), and address, phone numbers, and relatives all came back completely empty — only email worked. The code was guessing at section container ids that don't exist on NPD's real page; the actual ids live on each section's heading, with the data in a sibling element. Fixed and verified against a real saved profile page — now correctly extracts all of it.
