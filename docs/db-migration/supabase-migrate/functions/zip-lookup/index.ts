...
serve(async (req) => {
  ...
  try {
    ...
    if (!lookupError && existingZip) {
      ...
      // If profile_id is provided, update the qs_profile record
      if (profile_id) {
        const { error: updateError } = await supabaseClient
          .from('qs_profile')
          .update({ 
            city: existingZip.city, 
            state_id: existingZip.state_id,
            state_name: existingZip.state_name
          })
          .eq('id', profile_id)

        if (updateError) {
          console.error('Error updating qs_profile record:', updateError)
        }
      }
    ...
    if (profile_id) {
      ...
      if (updateError) {
        ...
        console.log(`Updated qs_profile record ${scan_id} with city and state`)
      }
    }
